...
```         
%let StDateTime = %sysfunc( datetime() );
%put &StDateTime;
...
data _null_;
   ...
   * マクロ変数に格納 ;
   call symput('StDates', trim(left(Date1)));
   call symput('StDate', trim(left(Date2)));
   call symput('StTimes', trim(left(Time1)));
   call symput('StTime', trim(left(Time2)));
run;
%put &StDates. &StDate. &StTimes. &StTime.;
```
